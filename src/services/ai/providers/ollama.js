...
async function* stream({ baseUrl, model, systemPrompt, history, prompt, images, temperature, maxTokens, usageOut, useMcp = true, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs }) {
...
async function complete({ baseUrl, model, systemPrompt, history, prompt, images, temperature, maxTokens, useMcp = true, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs }) {
    const toolkit = await toolkitFor({ useMcp, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs });
    const rounds = roundsFor(toolkit);
    const { url, agents } = resolveEndpoint(baseUrl);
    const messages = buildMessages({ systemPrompt, history, prompt, images, model });
