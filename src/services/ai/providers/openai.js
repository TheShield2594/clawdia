...
async function* stream({ apiKey, model, systemPrompt, history, prompt, images, temperature, maxTokens, visionCapable, baseURL, defaultHeaders, usageOut, useMcp = true, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs }) {
    const toolkit = await toolkitFor({ useMcp, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs });
...
async function complete({ apiKey, model, systemPrompt, history, prompt, images, temperature, maxTokens, visionCapable, baseURL, defaultHeaders, useMcp = true, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs }) {
    const toolkit = await toolkitFor({ useMcp, mcpServers, onToolEvent, mcpConfirm, confirmTool, elicit, toolBudget, botTools, botToolsOnly, maxRounds, turnBudgetMs });
    const rounds = roundsFor(toolkit);
    const client = new OpenAI({ apiKey, baseURL, defaultHeaders });
    const messages = buildMessages({ systemPrompt, history, prompt, images, model, visionCapable });
