...
async function handleTrickOrTreat(interaction) {
    ...
    if (isTrick) {
        ...
        // The candy and the candy bag are credited on their own keyed writes
        // after the save (below), not folded into `user` — see there.
    ...
    // activities. A trick is a negative delta, which commitBalanceDelta leaves
    // unkeyed. The save also inserts the document for a first-time player, so the
    // currency and item credits below always have a doc to land on.
    await saveWithBalanceDelta(User, user, balanceAtLoad, {
        service: 'trickortreat',
        jobName: 'knockPayout',
        guildId: interaction.guild.id,
        payoutKey: eventActivityPayoutKey('trickortreat', interaction.id, 'coins'),
    });
    if (!isTrick) {
        await creditActivityReward(embed, {
            filter: { userId: interaction.user.id, guildId: interaction.guild.id },
            activity: 'trickortreat',
            interactionId: interaction.id,
            currencyId: getEventCurrencyId(guildSettings),
            currencyAmount: CANDY_REWARD,
            currencyLabel: 'Candy',
            itemId: 'candy_bag',
            itemLabel: 'Candy Bag',
        });
...
module.exports = { handleTrickOrTreat };
