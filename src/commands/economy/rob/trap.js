...
// `/rob trap set` / `/rob trap status` — the tripwire that fires when someone
// robs you (#1022). This was `/trap` before the fold; the arming and the status
// read are unchanged, only the command path they live under and the copy that
// names it.
...
async function execute(interaction) {
    ...
    if (sub === 'status') {
        ...
        if (trapActive) {
            ...
            embed.setDescription(`You have **no active trap**.\n\nUse \`/rob trap set\` to arm one for **${currency}${TRAP_COST.toLocaleString()}**.`);
        }
    ...
    const embed = new EmbedBuilder()
        ...
        .setFooter({ text: 'Only one trap at a time. Use /rob trap status to check.' })
        .setTimestamp();

    return interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
}

module.exports = { execute, TRAP_COST };
