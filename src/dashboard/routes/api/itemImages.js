...
const { checkAuth, checkGuildAccess, checkWriteRateLimit, checkImageReadRateLimit } = require('../../lib/middleware');
...
router.get('/item-image/shop/:guildId/:itemId', checkAuth, checkGuildAccess, checkImageReadRateLimit, async (req, res) => {
    try {
...
router.get('/item-image/activity/:guildId/:itemId', checkAuth, checkGuildAccess, checkImageReadRateLimit, async (req, res) => {
    const { guildId, itemId } = req.params;
    try {
