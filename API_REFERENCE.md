...
| `GET` | `/api/v1/guild/:guildId/ai/usage` | session, guild admin | Token and request usage for the last `?days=` (1-90, default 14), plus the configured rate limits |
...
| `POST` | `/api/v1/guild/:guildId/mcp-servers/:name/test` | session, guild admin, write limit | Makes a real request through one MCP server and reports whether it connected |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/item-image/shop/:guildId/:itemId` | public | Serves a guild shop item's image |
...
| `GET` | `/api/v1/item-image/activity/:guildId/:itemId` | public | Serves a guild's activity item image, falling back to the shared pre-#561 one |
