...
// README's command list was hand-maintained and had drifted: 26 registered
// commands missing from it, and a documented `/buy` that has never been a
// command of its own (#712). The list is generated now, and this is what keeps
// it generated — adding a command file turns `npm test` red until
// `npm run docs:commands` has been run, instead of the drift being noticed by a
// reader some months later.
...
describe('README command reference', () => {
    ...
    test('keeps the markers the generator writes between', () => {
        ...
        expect(readme).toContain(BEGIN);
        expect(readme).toContain(END);
        expect(readme.indexOf(BEGIN)).toBeLessThan(readme.indexOf(END));
    });
    ...
    test('regenerating leaves the surrounding prose alone', () => {
        const readme = fs.readFileSync(README_PATH, 'utf8');
        const rewritten = replaceBlock(readme, 'placeholder');
