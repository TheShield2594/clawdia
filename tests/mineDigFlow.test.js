...
test('the prompt shows the survey and a button per rung, and the result offers Dig again', async () => {
    ...
    expect(final.embeds[0].data.description).toMatch(/Dug \*\*Steady\*\*/);
    expect(final.embeds[0].data.description).toMatch(/Next dig <t:\d+:R>/);
...
test('passing intensity skips the prompt, and a Careful dig costs no wear', async () => {
    ...
    expect(lastEdit(calls).embeds[0].data.title).not.toContain('Fled');
});
...
test('a Wooden Pickaxe can dig out of a cave-in: stamina for the ore', async () => {
    ...
    const final = lastEdit(calls).embeds[0].data;
...
test('blasting a Reckless cave-in spends three charges', async () => {
    ...
    expect(lastEdit(calls).embeds[0].data.description).toContain('You blasted clear (3 charges)');
});
