...
_Generated by `npm run docs:commands` from the loaded command set — 90 entries. Edit the commands, not this list._
...
- `/casino` — Play casino games: blackjack, coinflip, crash, cupgame, dice, higherlower, keno, poker, roulette, s…
...
- `/event` — Limited-time events: play the current one, or manage it.
...
- `/rob` — Rob a member, scout their defences, or arm a tripwire on your own wallet.
- `/season` — Season pass and economy season commands.
- `/shop` — Browse and buy items from the server, hunt, fish and mine shops
...
- `/trade` — Trade coins and/or an item with another member — both sides confirm before anything moves.
...
- `/ticket` — Open a private ticket with the moderators, or manage the ticket system
...
- `/rank` — View a rank card, or the server’s XP settings.
...
- `/mydata` — Export or delete the data Clawdia stores about you
