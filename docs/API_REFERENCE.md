...
| Item-image reads | 240 / minute | session, or IP when unauthenticated |

All answer `429 {"error": "Too many requests. Please slow down."}`. The read
...
The item-image `GET`s carry a read limiter of their own on top of the
router-wide one (`image read limit` in the table). They are `<img>` subresources
a single page loads dozens of at once — the activity-items page renders the whole
catalogue — so charging them to the write budget would keep an admin from saving,
and to the shared read budget would count them twice and starve the page's own
data reads. Their budget is separate and its ceiling higher for that reason.
...
resolve lookups each cost a Discord call, and `/stats` and `/insights` each run
collection-wide aggregations, so all of them are charged to the write budget as
well as the read one — both because they are expensive and because it puts a
rate limiter in the handler's own chain, which the router-wide read limiter,
applied through a conditional `router.use`, does not (CodeQL reads the chain, not
the wrapper). The ceilings are well above what the dashboard itself asks for — a
page load fires a handful of requests, not a hundred — and exist because those
reads are expensive.
...
- **image read limit** — the two item-image `GET`s carry a read limiter of
  their own (`checkImageReadRateLimit`, 240/minute on a separate budget) rather
  than relying on the router-wide read limit alone: they are `<img>`
  subresources a page loads in bulk, so they neither draw on the write budget
  nor double-count the shared read one
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/item-image/shop/:guildId/:itemId` | session, guild admin, image read limit | Serves a guild shop item's image |
...
| `GET` | `/api/v1/item-image/activity/:guildId/:itemId` | session, guild admin, image read limit | Serves a guild's activity item image, falling back to the shared pre-#561 one |
