...
_Generated by `npm run docs:api` from `src/dashboard/routes/api/` — 51 endpoints. Edit the routes and their comments, not this table._
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/stats` | session, guild admin, write limit | The dashboard's headline numbers for a guild: members, messages, coins in circulation, top levels and average XP |
| `GET` | `/api/v1/guild/:guildId/insights` | session, guild admin, write limit | Derived analytics: 7 and 30 day retention, activity by hour, and command usage |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/knowledge-base` | session, guild admin, write limit | One page of the guild's knowledge base entries, newest first |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/summary-jobs` | session, guild admin, write limit | One page of the guild's scheduled channel summary jobs, oldest first |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/daily-digest` | session, guild admin, write limit | The guild's AI daily digest settings |
...
| `GET` | `/api/v1/guild/:guildId/ai/usage` | session, guild admin, write limit | Token and request usage for the last `?days=` (1-90, default 14), plus the configured rate limits and what is left of the monthly ceiling (#831) |

### `mcpServers.js`

| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/mcp-servers` | session, guild admin, write limit | The guild's MCP servers, the operator's global ones, the presets, and whether editing is allowed at all |
...
| `GET` | `/api/v1/guild/:guildId/mcp-servers/usage` | session, guild admin, write limit | What the guild's MCP connections have actually been doing |
...
| `GET` | `/api/v1/mcp/oauth/callback` | session, write limit | The redirect back from the authorization server, exchanging the code for a stored grant |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/item-image/shop/:guildId/:itemId` | session, guild admin, write limit | Serves a guild shop item's image |
...
| `GET` | `/api/v1/item-image/activity/:guildId/:itemId` | session, guild admin, write limit | Serves a guild's activity item image, falling back to the shared pre-#561 one |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/cases` | session, guild admin, write limit | One page of moderation cases, filterable by `?type=` and `?status=` |
...
| `GET` | `/api/v1/guild/:guildId/sanctions/active` | session, guild admin, write limit | Up to 200 active bans and 200 active timeouts, read live from Discord |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/economy/stats` | session, guild admin, write limit | Economy overview: richest members by net worth, coins in circulation, the count of members who worked, claimed a daily or fished in the last 7 days, and the top commands |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/leveling/leaderboard` | session, guild admin, write limit | One page of members ranked by level then XP, 25 to a page |
