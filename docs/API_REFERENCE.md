...
The one exception is the **public pages** under `/s` (#1018) — a read-only,
opt-in-per-guild server page and player card that take no session at all. They
are a separate router, not part of `/api/v1`, and are listed in their own
...
_Generated by `npm run docs:api` from `src/dashboard/routes/api/` — 58 endpoints. Edit the routes and their comments, not this table._
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/session` | session | Whether this session still authenticates: 200 when it does, 401 when it does not, and never a redirect — the unambiguous signal the banner confirms against |
...
| `PATCH` | `/api/v1/guild/:guildId/rss/:index` | session, guild admin, write limit | Sets one feed's delivery options: keyword filters, a role to ping, and the message line posted above each item's embed |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `POST` | `/api/v1/guild/:guildId/social/validate` | session, guild admin, write limit | Resolves what the admin pasted to a feed URL and confirms it parses, before they commit to it |
| `POST` | `/api/v1/guild/:guildId/social/add` | session, guild admin, write limit | Subscribes a channel to a social account |
| `DELETE` | `/api/v1/guild/:guildId/social/:index` | session, guild admin, write limit | Unsubscribes from the account at a position in the guild's social list |
...
| `GET` | `/api/v1/guild/:guildId/members/:userId/ledger` | session, guild admin, write limit | One page of a member's transaction ledger, newest first, plus every owed payout still outstanding for them |
| `DELETE` | `/api/v1/guild/:guildId/members/:userId/data` | session, guild admin, Administrator, write limit | Erase a member's data on request (#1013) |
...
| `POST` | `/api/v1/guild/:guildId/sanctions/unban/:userId` | session, guild admin, Ban Members, write limit | Lifts a ban, attributing it to the dashboard user, and writes an audit entry |
| `POST` | `/api/v1/guild/:guildId/sanctions/untimeout/:userId` | session, guild admin, Moderate Members, write limit | Clears a member's timeout, attributing it to the dashboard user, and writes an audit entry |

### `tickets.js`

| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/tickets` | session, guild admin | List the tickets currently open on this guild, newest first, with the opener and claimer tags resolved |
...
| `POST` | `/api/v1/guild/:guildId/economy/adjust` | session, guild admin, Administrator, write limit | Gives, takes, resets, freezes or unfreezes one member's balance, and writes an audit entry |
...
The routes below are the dashboard's only unauthenticated ones (#1018). They are
not under `/api/v1` — they are a separate router mounted at `/s`
(`src/dashboard/routes/public.js`) — so they render HTML pages and a share image
rather than the JSON envelope the API uses, and they carry no `checkAuth` at all.

They are gated instead by two opt-ins, and every gate that fails answers the same
`404` so the URL space cannot be probed:

- a guild serves these routes only while its **`publicPage.enabled`** toggle is on
  (off by default), set from the dashboard's Public Page panel;
- a player card is served only for a member who has run **`/profile public on`**;
  a member who has not opted in, has no record, or is not in the guild is the same
  `404` as a guild whose page is off.

No response here returns a Discord user id for a member who has not opted in:
leaderboards name people by display name, and only an opted-in member's own id
(and a link to their card) appears. Each route is rate-limited per IP and sent
with a `public`, cacheable `Cache-Control`, since unlike every authenticated
response they may sit in a shared cache. A guild may also set a vanity `slug`, and
the pages are then reachable at `/s/<slug>` as well as `/s/<guildId>`.

<!-- BEGIN GENERATED PUBLIC ROUTES — npm run docs:api -->

_Generated by `npm run docs:api` from `src/dashboard/routes/public.js` — 3 routes. These are the only public, session-free routes; everything above needs a session._

| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/s/:id` | public | The public server page: the guild's header, the weekly champion race, the top ten on each leaderboard the admin ticked, the active seasonal event and district funding |
| `GET` | `/s/:id/u/:userId` | public | The public player card: the same figures /profile and /showcase render, as HTML |
| `GET` | `/s/:id/u/:userId/card.png` | public | The player card's Open Graph image, drawn per request so a shared link unfurls in Discord |

<!-- END GENERATED PUBLIC ROUTES -->
