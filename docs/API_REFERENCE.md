...
A few reads also carry a per-route `read limit` on top of the router-wide one:
the `/stats` and `/insights` aggregations and the item-image reads. These are the
routes CodeQL's `js/missing-rate-limiting` query flags, and it only recognises a
limiter that comes from a rate-limiting package — not the shared limiter above —
so they use an `express-rate-limit` limiter, placed before the guild-access check
so the protection is one the analyser (and a reader) can see on the route itself.
...
- **read limit** — a per-route read limiter in addition to the router-wide one
  (`statsReadRateLimit` on `/stats` and `/insights`, `imageReadRateLimit` on the
  item-image reads). These are `express-rate-limit` limiters placed before the
  guild-access check, so the routes CodeQL flags carry a limiter it recognises
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/guild/:guildId/stats` | session, guild admin, read limit | The dashboard's headline numbers for a guild: members, messages, coins in circulation, top levels and average XP |
| `GET` | `/api/v1/guild/:guildId/insights` | session, guild admin, read limit | Derived analytics: 7 and 30 day retention, activity by hour, and command usage |
...
| Method | Path | Requires | Summary |
| --- | --- | --- | --- |
| `GET` | `/api/v1/item-image/shop/:guildId/:itemId` | session, guild admin, read limit | Serves a guild shop item's image |
...
| `GET` | `/api/v1/item-image/activity/:guildId/:itemId` | session, guild admin, read limit | Serves a guild's activity item image, falling back to the shared pre-#561 one |
