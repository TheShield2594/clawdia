...
treatment the nine long-stable subsystems got. Seventeen passes have landed under
...
map views in v4.13.8, and the `/explore` views in v4.13.9 — and between them they found the same defect on
path after path: a
...
Seventeen passes have landed against it — `/duel` escrow and the crew splits, the
...
1. the rest of the non-payout surface: the gathering surface pass 9 did not need
   to touch (the map views and `/explore`'s views are done — passes 16 and
   17), and the season pass's non-reward surface
